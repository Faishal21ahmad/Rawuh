...
<div class="flex flex-auto overflow-x-auto flex-nowrap object-cover">
    @foreach ($pesanan as $item)
    @if ($item->statuspesanan == 'diterima')
    <div class="relative w-full max-w-sm m-2 bg-white border rounded-lg shadow-md px-8 pt-8 pb-44 dark:bg-gray-800 dark:border-gray-700">
            ...
            <table class="w-full mt-5 mb-5 text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    @if ($i->codepesan == $item->codepesan) 
                        <tbody>
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                <th scope="row" class="px-4 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $i->menu->menu }}</th>
                                <td class="px-4 py-4">{{ $i->jumlah }}</td>
                                <td class="px-4 py-4">{{ $i->subharga }}</td>
                            </tr>
                        </tbody>
                    @endif
                @endforeach
                <tbody>
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-4 py-4">Total Harga</td>
                        <td class="px-4 py-4">{{ $item->totalharga }}</td>
                    </tr>
                </tbody>
            </table>
        ...
        <form action="/pesanan/{{ $item->id }}" method="POST" enctype="multipart/form-data">>
            @method('put')
            @csrf
            ...
            <button class="inline-flex absolute inset-x-0 bottom-0 m-5 py-3 text-sm font-medium text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 "><p class="mx-auto flex items-center">Terima</p></button>
        </form>
    </div>
    @endif
    @endforeach
