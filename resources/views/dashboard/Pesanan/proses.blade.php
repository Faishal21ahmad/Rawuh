...
<div class="flex flex-auto overflow-x-auto flex-nowrap object-cover ">
    ...
    @if ($item->statuspesanan == 'diproses')
    <div class="relative w-full max-w-sm m-2 bg-white border rounded-lg shadow-md px-8 pt-8 pb-44 dark:bg-gray-800 dark:border-gray-700">
        ...
        <h5 class="text-xl my-1 font-bold leading-none text-gray-900 dark:text-white">Meja Nomor : {{ $item->meja_id }}</h5>
        ...
        <form action="/pesanan/{{ $item->id }}" method="POST" enctype="multipart/form-data">>
            ...
            <div class="items-center mb-4 hidden">
                <input id="statuspesanan" type="radio" value="selesai" name="statuspesanan" checked>
                <label for="statuspesanan">Default radio</label>
            </div>
            <button class="inline-flex absolute inset-x-0 bottom-0 m-5 py-3 text-sm font-medium text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 "><p class="mx-auto flex items-center">Selesaikan</p></button>
        </form>
    </div>
    @endif
    @endforeach
